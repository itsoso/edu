...
ESSAY_OCR_MULTI_PROMPT = """以下 {n} 张图片是同一篇作文的连续页面 (按顺序: 第 1 页, 第 2 页, ...). 请识别全部页面并按顺序拼接为一篇完整作文。

要求:
- 严格按图片顺序拼接, 不要遗漏页面或段落
- 跨页同一段落要合并 (前页末尾未结束的句子, 接到后页开头, 不要插入换行)
- 段落之间用单个换行
- 逐字还原, 包括标点符号
- 涂改/修正以最终版本为准
- 只提取学生写的内容, 忽略印刷格线、页码、装订标记
- 标题只在首页有, 单独提取一次

严格返回 JSON:

{{
  "title": "作文标题 (从首页识别, 否则填 null)",
  "content": "拼接后的作文正文全文 (所有 {n} 页)",
  "confidence": 0.0~1.0,
  "issues": ["识别可能不准确的部分说明"]
}}
...
SCAN_SOLVE_PROMPT = """请仔细看这张题目照片, 识别题目并给出完整解答。

要求:
- question_text: 用文字 + LaTeX ($...$) 还原题目, 数学符号必须用 LaTeX
- subject: 学科(数学/物理/化学/英语/语文/生物/历史/地理/政治), 看不出填"其他"
- knowledge_point: 涉及的核心知识点(简短, 如"一元二次方程"/"光的折射"/"化学反应配平")
- difficulty: easy / medium / hard
- answer: 最终答案(简洁, 一两句话, 包含 LaTeX)
- solution_steps: Markdown + LaTeX 格式的完整解题步骤. 至少 3 步, 让初中生能跟上, 不要跳步
- common_mistakes: 这道题学生常见的 1-2 个错误点 (用作"错因"提示)

如果照片里没有题目 (空白/模糊/无关), 返回 question_text="" 和 error 字段说明。
...
ESSAY_MODEL_PROMPT = """你是一位资深的初中语文老师。请根据用户确认的作文元数据，写一篇**同主题、同类型**的优秀范文供学生学习参考。

作文主题: {topic}
作文类型: {essay_type}
目标学生作文字数: {word_count} 字

要求:
- 字数: 与学生原文接近 ({target_words} 字左右), ±100 字内
- 中考作文一类卷标准, 但要"接地气" — 不是堆砌辞藻, 是真实可学的好作文
- 结构清晰: 凤头、猪肚、豹尾
- 至少有一处可圈可点的细节描写或巧妙构思
- 不接收也不推断学生原文内容, 写一篇全新的、独立的好文
- 主题字段仅用于把握方向, 不是完整命题
...
{{
  "title": "范文标题",
  "content": "范文正文 (使用 \\n 分段)",
  "highlights": ["可学习要点 1 (一句话, 比如 '用对话推动情节')", "要点 2", "要点 3"],
  "structure_note": "结构分析, 一两句话说清楚开头-发展-高潮-结尾"
}}
